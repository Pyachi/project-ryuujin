...
public abstract class Crate extends GameObject {
    ...
    public Crate(Vec2 loc, int value) {
        this.value = value;
    ...
    public abstract Crate getMergedCrate(Vec2 loc, Crate crate);
    ...
    public boolean canMove(int x, int y) {
        ...
        boolean collision = getCollisions(Player.class).stream().anyMatch(it -> it.getGrabbedCrate() != this) ||
                ...
                !canInteractWith(it) && !it.canInteractWith(this));
        move(-x, -y);
        return !collision;
    }
    
    public boolean isGrabbed() {
        return getLevel().getObjects().stream().filter(Player.class::isInstance).map(Player.class::cast).anyMatch(it ->
                it.getGrabbedCrate() == this);
    }
    
    private void generateNew(Crate crate) {
        if (isDead()) return;
        var location = getLocation();
        if (getLevel().getObjects().stream().filter(Player.class::isInstance).anyMatch(it ->
                ((Player) it).getGrabbedCrate() == this)) location = crate.getLocation();
    ...
    public void paint(Graphics2D g) {
        ...
        super.paint(g);
        g.setFont(Game.font()
                .deriveFont((float) 16 * getLevel().getScale() * 0.8F /
