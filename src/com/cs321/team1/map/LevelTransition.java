...
/**
 * GameSegment representing an animation between levels
 */
public class LevelTransition implements GameSegment {
    ...
    /**
     * Creates a LevelTransition from {@param from} to {@param to}
     *
     * @param from The level being transitioned from
     * @param to   The level being transitioned to
     */
    ...
    public BufferedImage render() {
        ...
        if (lvlImage != null) g.drawImage(lvlImage,
                (image.getWidth() - lvlImage.getWidth()) / 2,
                (image.getHeight() - lvlImage.getHeight()) / 2,
                lvlImage.getWidth(),
                lvlImage.getHeight(),
                null);
        if (tick <= 20) g.fillRect((int) (image.getWidth() - (image.getWidth() * (tick / 20.0))),
                0,
                (int) (image.getWidth() * (tick / 20.0)),
                image.getHeight());
        else g.fillRect(0, 0, (int) (image.getWidth() * ((40.0 - tick) / 20.0)), image.getHeight());
        return image;
    }
